...
scale_factor = 0.001  # Adjust this value to get the desired scale
smpl_data = {
    'v_template': verts_all[0] * scale_factor,  # Scale down the template vertices
    'shapedirs': np.zeros((verts_all.shape[1], 3, 10)),  # Placeholder for shape directions
    'J_regressor': J_regressor.cpu().numpy(),  # Use the actual J_regressor
    'weights': np.zeros((verts_all.shape[1], reg3d_all.shape[1])),  # Placeholder for weights
    'kintree_table': np.zeros((2, reg3d_all.shape[1]), dtype=int),  # Kinematic tree
    ...
    'J': reg3d_all[0] * scale_factor,  # Scale down the first frame's joint positions
}

# Store the initial joint positions in kintree_table
smpl_data['kintree_table'][1, :] = np.arange(reg3d_all.shape[1])  # Joint indices
smpl_data['kintree_table'][0, 1:] = np.arange(reg3d_all.shape[1] - 1)  # Parent indices (assuming root is the first joint)
smpl_data['kintree_table'][0, 0] = -1  # Root has no parent
