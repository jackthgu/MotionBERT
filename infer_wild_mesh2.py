...
def export_smpl_parameters(smpl_output, output_path):
    # Extract the SMPL parameters from the output
    poses = smpl_output.body_pose.detach().cpu().numpy()  # Pose parameters (shape: [N, 72])
    betas = smpl_output.betas.detach().cpu().numpy()      # Shape parameters (shape: [N, 10])
    translations = smpl_output.global_orient.detach().cpu().numpy()  # Global orientation / translation
    ...
    smpl_data = {
        ...
        'betas': betas,
        'trans': translations,
        'mocap_framerate': 30.0,  # Example framerate, replace with actual if different
        'gender': 'male'  # Set gender if known (can be 'male' or 'female')
    }
...
with torch.no_grad():
    for batch_input in tqdm(test_loader):
        ...
        export_smpl_parameters(output_flip_smpl, opts.out_path)
...
smpl_data = {
    'v_template': verts_all[0],  # Use the first frame as the template
    'shapedirs': np.zeros((verts_all.shape[1], 3, 10)),  # Placeholder for shape directions
    'J_regressor': np.zeros((reg3d_all.shape[1], verts_all.shape[1])),  # Placeholder for joint regressor
    'weights': np.zeros((verts_all.shape[1], reg3d_all.shape[1])),  # Placeholder for weights
    'kintree_table': np.zeros((2, reg3d_all.shape[1]), dtype=int),  # Placeholder for kinematic tree
    'f': np.zeros((verts_all.shape[1] // 3, 3), dtype=int),  # Placeholder for face indices
    'betas': np.zeros(10),  # Placeholder for shape parameters
}

# Save the dictionary as an NPZ file
np.savez('my_smpl_output.npz', **smpl_data)
